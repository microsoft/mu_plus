  ...
  )
{
  ...
  for (i = 0; i < ARRAY_SIZE (gReadyToBootPolicies); i++) {
    Status = RegisterVarStateVariablePolicy (
               VariablePolicy,
               gReadyToBootPolicies[i].Namespace,          gReadyToBootPolicies[i].Name,
               gReadyToBootPolicies[i].MinSize,            gReadyToBootPolicies[i].MaxSize,
               gReadyToBootPolicies[i].AttributesMustHave, gReadyToBootPolicies[i].AttributesCantHave,
               &gMuVarPolicyDxePhaseGuid,  READY_TO_BOOT_INDICATOR_VAR_NAME, PHASE_INDICATOR_SET
               );
    if (EFI_ERROR (Status)) {
  ...
  for (i = 0; i < ARRAY_SIZE (gMailBoxPolicies); i++) {
    Status = RegisterBasicVariablePolicy (
               VariablePolicy,
               gMailBoxPolicies[i].Namespace,          gMailBoxPolicies[i].Name,
               gMailBoxPolicies[i].MinSize,            gMailBoxPolicies[i].MaxSize,
               gMailBoxPolicies[i].AttributesMustHave, gMailBoxPolicies[i].AttributesCantHave,
