  ...
  )
{
  ...
  if (CperHdr->SectionCount != CPER_HDR_SEC_CNT) {
    Status = EFI_PROTOCOL_ERROR;
    UT_LOG_ERROR (
      "CPER Header section count mismatch: has: %d, expect: %d.",
  ...
  if (CperHdr->ErrorSeverity != ErrorSeverity) {
    Status = EFI_PROTOCOL_ERROR;
    UT_LOG_ERROR (
      "CPER Header error severity mismatch: has: %d, expect: %d.",
      ...
      (CompareMem (&CperHdr->PlatformID, PcdGetPtr (PcdDeviceIdentifierGuid), sizeof (EFI_GUID))))
  {
    Status = EFI_PROTOCOL_ERROR;
    UT_LOG_ERROR (
      "CPER Header Creator Id incorrect: has: %g, validation bits: %X.",
       ...
       ((CperHdr->ValidationBits & EFI_ERROR_RECORD_HEADER_PARTITION_ID_VALID) == 0)))
  {
    Status = EFI_PROTOCOL_ERROR;
    UT_LOG_ERROR (
      "CPER Header Partition Id incorrect: has Guid: %g, validation bits: %X.",
      CperHdr->PartitionID,
      CperHdr->ValidationBits
      );
    goto Cleanup;
  ...
  if (CperHdr->RecordLength != TotalSize) {
    ...
    UT_LOG_ERROR (
      "CPER Header record length incorrect: has: %08X, expect: %08X.",
  ...
  if (CompareGuid (&CperHdr->CreatorID, &gMsWheaReportServiceGuid) == FALSE) {
    Status = EFI_PROTOCOL_ERROR;
    UT_LOG_ERROR (
      "CPER Header Creator ID mismatch: has: %g, expect: %g.",
      CperHdr->CreatorID,
  ...
  if (CompareGuid (&CperHdr->NotificationType, &gEfiEventNotificationTypeBootGuid) == FALSE) {
    Status = EFI_PROTOCOL_ERROR;
    UT_LOG_ERROR (
      "CPER Header Notification Type mismatch: has: %g, expect: %g.",
  ...
  if (CperHdr->Flags != EFI_HW_ERROR_FLAGS_PREVERR) {
    Status = EFI_PROTOCOL_ERROR;
    UT_LOG_ERROR (
      "CPER Header Error Flags mismatch: has: %08X, expect: %08X.",
      CperHdr->Flags,
  ...
  )
{
  ...
  if (CperSecDecs->SectionLength != (TotalSize - HW_ERR_REC_HEADERS_OFFSET)) {
    Status = EFI_BAD_BUFFER_SIZE;
    UT_LOG_ERROR (
      "CPER Section Descriptor length mismatch: has %08X, expects %08X.",
  ...
  if (CompareGuid (&CperSecDecs->SectionType, &gMuTelemetrySectionTypeGuid) == FALSE) {
    Status = EFI_PROTOCOL_ERROR;
    UT_LOG_ERROR (
      "CPER Section Descriptor Section Type mismatch: has: %g, expect: %g.",
  ...
  if (CperSecDecs->Severity != ErrorSeverity) {
    Status = EFI_PROTOCOL_ERROR;
    UT_LOG_ERROR (
      "CPER Section Descriptor Error severity mismatch: has: %08X, expect: %08X.",
  ...
  )
{
  ...
  if (CompareMem (&MuTelemetrySectionData->ComponentID, &gEfiCallerIdGuid, sizeof (EFI_GUID))) {
    Status = EFI_PROTOCOL_ERROR;
    UT_LOG_ERROR (
      "Mu Telemetry system Id mismatch: %d.",
      ...
      CompareMem (&MuTelemetrySectionData->SubComponentID, LibraryId, sizeof (EFI_GUID)))
  {
    Status = EFI_PROTOCOL_ERROR;
    UT_LOG_ERROR (
      "Mu Telemetry subsystem Id mismatch: %g.",
  ...
  if (MuTelemetrySectionData->ErrorStatusValue != ErrorStatusCodeValue) {
    Status = EFI_PROTOCOL_ERROR;
    UT_LOG_ERROR (
      "Mu Telemetry ErrorStatusValue mismatch: has: %08X, expect %08X.",
      MuTelemetrySectionData->ErrorStatusValue,
  ...
  if (MuTelemetrySectionData->AdditionalInfo1 != AdditionalInfo1) {
    Status = EFI_PROTOCOL_ERROR;
    UT_LOG_ERROR (
      "Mu Telemetry AdditionalInfo1 mismatch: has: %16X, expect %16X.",
      MuTelemetrySectionData->AdditionalInfo1,
      AdditionalInfo1
      );
    goto Cleanup;
  }
  ...
  if (MuTelemetrySectionData->AdditionalInfo2 != AdditionalInfo2) {
    Status = EFI_PROTOCOL_ERROR;
    UT_LOG_ERROR (
      "Mu Telemetry AdditionalInfo2 mismatch: has: %16X, expect %16X.",
      MuTelemetrySectionData->AdditionalInfo2,
      AdditionalInfo2
      );
    goto Cleanup;
  }
  ...
  )
{
  ...
  if ((Status == EFI_NOT_FOUND) && (MsWheaContext->TestId == TEST_ID_BOUNDARY)) {
    ...
    DEBUG ((
      DEBUG_INFO,
      "%a Boundary test has Not Found error %s %08X %08X\n",
      __FUNCTION__,
      VarName,
    ...
    UT_LOG_WARNING (
      "Variable service read %s returns %08X at Test No. %d.",
      VarName,
      Status,
      TestIndex
      );
  ...
  if (EFI_ERROR (Status) != FALSE) {
    UT_LOG_ERROR (
      "Variable service read %s returns %08X, expecting %08X.",
      VarName,
      Status,
  ...
  Status      = MsWheaVerifyCPERSecDesc (
                  Context,
                  CperSecDecs,
                  ErrorSeverity,
                  ErrorStatusCodeValue,
  ...
  Status       = MsWheaVerifyMuTelemetryErrorData (
                   Context,
                   MuTelSecData,
                   LibraryId,
                   ErrorStatusCodeValue,
  ...
  )
{
  ...
  for (TestIndex = 0; TestIndex < UNIT_TEST_SINGLE_ROUND; TestIndex++) {
    ...
    Status = MsWheaVerifyFlashStorage (
               Context,
               TestIndex,
               UNIT_TEST_ERROR_CODE | TestIndex,
  ...
  )
{
  ...
  Status = ReportStatusCodeWithExtendedData (
  ...
  )
{
  ...
  if (Status != EFI_NOT_FOUND) {
    utStatus = UNIT_TEST_ERROR_TEST_FAILED;
    UT_LOG_ERROR (
      "Stress test case failed as payload returns %08X, expecting %08X.",
      Status,
      EFI_NOT_FOUND
      );
    goto Cleanup;
