...
#
# This is to build the SharedCrypto binaries for NuGet publishing
##
import os
import sys
...
sys.path.append(os.path.join(os.path.dirname(__file__), "..",
                             "MU_BASECORE", "NetworkPkg", "SharedNetworking"))
...
class SettingsManager(UpdateSettingsManager, CiSetupSettingsManager, BinaryBuildSettingsManager):
    ...
    def GetActiveScopes(self):
        ...
        if (GetHostInfo().os == "Linux"
            ...
            and self.ToolChainTagCacheValue is not None
                and self.ToolChainTagCacheValue.upper().startswith("GCC")):
    ...
    def _CollectNuget(self):
        ...
        shared_networking_build_dir = os.path.realpath(
            os.path.join(rootDir, "Build", "SharedCryptoPkg"))
        ...
        build_dir_txt_search = os.path.join(
            shared_networking_build_dir, "**", f"{self.target}*", "**", "BUILD_REPORT.txt")
    ...
    def GetPackagesPath(self):
        ...
        pp = [os.path.join(ws, x) for x in self.rr]
        return pp

    def GetModulePkgsPath(self):
        ''' get module packages path '''
        pp = self.GetPackagesPath()
        ws = self.GetWorkspaceRoot()
        return ";".join(os.path.join(ws, pkg_name) for pkg_name in pp)
    ...
    def GetConfigurations(self):
        ...
        for target in TARGETS:
            for arch in ARCHS:
                self.target = target
                self.arch = arch
                print(f"Starting on {target} {arch}")
                data = {"TARGET": target, "TARGET_ARCH": arch, "ACTIVE_PLATFORM": "SharedCryptoPkg/SharedCryptoPkg.dsc" }
                yield(data)

    def GetConfigurationLabel(self):
        env = shell_environment.GetBuildVars()
        target = env.GetValue("TARGET")
        arch = env.GetValue("TARGET_ARCH")
        label_parts = [target, arch]
...
if __name__ == "__main__":
    import SharedCryptoSettings
    SharedCryptoSettings.main()  # otherwise we're in __main__ context