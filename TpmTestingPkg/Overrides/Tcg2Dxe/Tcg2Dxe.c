  ...
  )
{
  ...
  if (mTpmReplayConfig.ActivePcrs.Data & (1 << PCRIndex)) {
    DEBUG ((
      DEBUG_INFO,
      "[%a][TPM Replay] - Skipping %a action measurement to PCR %d.\n",
      __func__,
  ...
  )
{
  ...
  if (mTpmReplayConfig.ActivePcrs.Pcrs.Pcr1) {
    DEBUG ((DEBUG_INFO, "[%a][TPM Replay] - Skipping handoff table measurement to PCR 1.\n", __func__));
    return EFI_SUCCESS;
  }
  ...
  )
{
  ...
  if (mTpmReplayConfig.ActivePcrs.Data & (1 << PCRIndex)) {
    DEBUG ((DEBUG_INFO, "[%a][TPM Replay] - Skipping separator measurement to PCR %d.\n", __func__, PCRIndex));
    return EFI_SUCCESS;
  }
  ...
  )
{
  ...
  if (mTpmReplayConfig.ActivePcrs.Data & (1 << PCRIndex)) {
    DEBUG ((DEBUG_INFO, "[%a][TPM Replay] - Skipping variable measurement to PCR %d.\n", __func__, PCRIndex));
    return EFI_SUCCESS;
  }
  ...
  )
{
  ...
  if (mTpmReplayConfig.ActivePcrs.Pcrs.Pcr7) {
    DEBUG ((DEBUG_INFO, "[%a][TPM Replay] - Skipping debugger measurement to PCR 7.\n", __func__));
    return EFI_SUCCESS;
  }
  ...
  )
{
  ...
  ConfigHob = GetFirstGuidHob (&gTpmReplayConfigHobGuid);
  if (ConfigHob != NULL) {
    ASSERT (GET_GUID_HOB_DATA_SIZE (ConfigHob) >= sizeof (TPM_REPLAY_CONFIG));
    return GET_GUID_HOB_DATA (ConfigHob);
  }
  ...
  )
{
  EFI_STATUS                       Status;
  EFI_EVENT                        Event;
  TPM_REPLAY_CONFIG                *TpmReplayConfig;  // MU_CHANGE - TPM Replay Feature
  VOID                             *Registration;
  ...
  TpmReplayConfig = GetTpmReplayConfig ();
  if (TpmReplayConfig != NULL) {
    ASSERT (TpmReplayConfig->Signature == TPM_REPLAY_CONFIG_SIGNATURE);
    CopyMem (&mTpmReplayConfig, TpmReplayConfig, TpmReplayConfig->HeaderLength);
  }
