  ...
  )
{
  ...
  TPM_REPLAY_CONFIG  *ReplayConfig;
  VOID               *ConfigHob;

  ConfigHob = GetFirstGuidHob (&gTpmReplayConfigHobGuid);
  if (ConfigHob != NULL) {
    ReplayConfig = GET_GUID_HOB_DATA (ConfigHob);
    ASSERT (ReplayConfig->Signature == TPM_REPLAY_CONFIG_SIGNATURE);
    ASSERT (GET_GUID_HOB_DATA_SIZE (ConfigHob) >= sizeof (ReplayConfig));

    if (ReplayConfig->ActivePcrs.Pcrs.Pcr7) {
      DEBUG ((
        DEBUG_INFO,
        "[%a][TPM Replay] - Skipping Secure Boot variable [%s] measurement to PCR 7.\n",
        __func__,
        (VariableName != NULL) ? VariableName : L"Unknown"
        ));
      return;
    }
  }
