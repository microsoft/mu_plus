...
STATIC TPM_REPLAY_CONFIG  mTpmReplayConfig;
  ...
  )
{
  ...
  if (mTpmReplayConfig.ActivePcrs.Pcrs.Pcr5) {
    DEBUG ((DEBUG_INFO, "[%a][TPM Replay] - Skipping GPT disk geometry measurement to PCR 5.\n", __func__));
  ...
  )
{
  ...
  if (mTpmReplayConfig.ActivePcrs.Data & (1 << Tcg2Event->Header.PCRIndex)) {
    if (ImageLoad->DevicePath != NULL) {
      DEBUG ((
        DEBUG_INFO,
        "[%a][TPM Replay] - Skipping PE/COFF image [%s] measurement to PCR %d.\n",
        __func__,
        ConvertDevicePathToText (ImageLoad->DevicePath, FALSE, TRUE) != NULL ? \
        ConvertDevicePathToText (ImageLoad->DevicePath, FALSE, TRUE) : L"?",
        Tcg2Event->Header.PCRIndex
        ));
    }
