...
#get workspace path
ws = os.path.dirname(os.path.dirname(sp)) #UEFI workspace will be parent of parent
pp = ws
...
class PlatformBuilder(UefiBuilder):
    ...
    def __init__(self, workspace, packagespath, args, ignore = None):
...
def RunPlatformTests(test_list, workpath = ws, packagepath = pp, ignore_list = None):
    pass
...
if __name__ == '__main__':
    ...
    #Unless they are specified in ignore file
        ...
        for File in DSCFiles:
            ...
            #Build Platform
            PB = PlatformBuilder(ws, pp, temp_argv, IgnoreList)
            starttime = time.time()
            retcode = PB.Go()
            if not "--skipbuild" in str(sys.argv).lower():
                if(retcode != 0):
