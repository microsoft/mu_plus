  ...
  )
{
  ...
  CaptionLabel = new_Label (
                   ControlOrigX,
                   ControlOrigY,
                   (DialogBounds.Right - ControlOrigX + 1 - ((DialogWidth * SWM_SS_DIALOG_CAPTION_X_PERCENT) / 100)),
  ...
  BodyLabel = new_Label (
                ControlOrigX,
                ControlOrigY,
                (DialogBounds.Right - ControlOrigX + 1 - ((DialogWidth * SWM_SS_DIALOG_RIGHT_PADDING_PERCENT) / 100)),
                (DialogBounds.Bottom - ControlOrigY + 1),                  // In theory we could take up the entire dialog.
                &FontInfo,
                &gMsColorTable.SingleSelectDialogTextColor,
                &gMsColorTable.SingleSelectDialogDialogBackGroundColor,
  ...
  mOptionListBox->Base.GetControlBounds (
                         mOptionListBox,
                         &ControlBounds
                         );
  ...
  do {
    ...
    while (EFI_SUCCESS == Status) {
      ...
      Status = this->WaitForEvent (
                       2,                                 // Two events (pointer and keyboard).
                       WaitEvents,
                       &Index,
                       0,
                       FALSE
  ...
  )
{
  ...
  //
  Status = CreateSingleSelectDialog (
             This,
             FrameRect,
             pTitleBarText,
             pCaptionText,
  ...
  *Result = ProcessDialogInput (
  ...
  if (SWM_MB_IDOK == *Result) {
    ...
    Status = mOptionListBox->GetSelectedCellIndex (
                               mOptionListBox,
                               &ReturnData
                               );
