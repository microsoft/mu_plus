  ...
  )
{
      ...
      ((ReturnData->TargetCell < 0)   || (ReturnData->TargetCell >= this->m_NumberOfCells)))
  {
    DEBUG ((
      DEBUG_INFO,
      "Ignoring input due to range error Sel=%d, Tgt=%d\n",
  ...
  )
{
  ...
  if (LocationNone != this->m_CaptureLocation) {
    // Capture Pointer in effect
  ...
  } else if (pInputState->State.TouchState.ActiveButtons == 0x01) {
    // First Touch event
  ...
  if (pInputState->State.TouchState.ActiveButtons == 0x00) {
    // Clear capture mode on any up event
    this->m_CaptureLocation = LocationNone;
  }
